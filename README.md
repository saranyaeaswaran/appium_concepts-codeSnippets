...
## Using Appium desktop Inspector to inspect the elements to get the properties:
		...
		a. From Appium desktop, invoke 'Start Inspector Session' (magnifying glass at right top corner)
		b. Once the session is invoked, click on 'Attached to session' > Hit Refresh icon to select the current running session > select the current session from dropdown > click Attach to Session
		c. This will get the screenshot of the application and from here we can inspect through the elements
